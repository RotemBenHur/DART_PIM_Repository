function [out_mux,inter,cyc_num_tot,MAGICs_num_tot] = MUX_b_1bit(a,b,sel,out)
% receives three vector inputs of single-bits elements (a, b and select) and returns
% a if select=0 and b otherwise, based on bitwise NOR operation

if (all(out))
    [inter(:,1),cyc_num(1),MAGICs_num(1)] = NOT_bitwise(sel,out(:,1));
    [inter(:,2),cyc_num(2),MAGICs_num(2)] = NOR2_bitwise(b,inter(:,1),out(:,2));
    [inter(:,3),cyc_num(3),MAGICs_num(3)] = NOR2_bitwise(a,sel,out(:,3));
